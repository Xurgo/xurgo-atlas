...
export async function validatePatch(
  ...
  try {
    ...
    if (normalized.changedFiles.length !== 1 || normalized.changedFiles[0] !== path) {
      return {
        valid: false,
        failureType: 'invalid',
        applyable: false,
        error:
          `Patch must modify only "${path}". ` +
          `Detected changed file(s): ${normalized.changedFiles.join(', ') || '(none)'}.`,
      };
    }
    ...
    return {
      ...
      applyable: false,
      error:
        `docs.propose_patch requires a complete unified diff patch. ` +
  ...
  const applyCheck = await gitStore.validatePatchApplyability(
    branch,
    normalizedPatch,
    [path],
  );
  ...
  try {
    newContent = applyUnifiedDiff(originalContent, normalizedPatch);
  } catch (err: unknown) {
    return {
      valid: false,
      failureType: 'invalid',
...
export function buildUnifiedDiffRequirementMessage(
  ...
  return (
    `docs.propose_patch requires a standard unified diff patch. ` +
    `${reasons[formatError.receivedFormat]} ` +
    `${formatError.hint} Accepted formats: full git-style unified diffs generated by git diff, ` +
    `or complete unified diffs with ---/+++ file headers and @@ hunks. Unsupported formats: ` +
    `prose-only text, OpenAI apply_patch envelopes, and truncated or corrupt hunks.`
  );
}
