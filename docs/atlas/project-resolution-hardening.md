...
This document tracks release hardening and follow-up edge cases for Xurgo Atlas project resolution and root/worktree safety discovered during the `0.1.0` release candidate process. It applies to the Xurgo Atlas source repository itself and is not a template for downstream projects.
...
- Root/worktree identity is not yet a first-class binding surface; copied checkouts and secondary worktrees still need a deliberate model before Atlas can rely on them for writes.
...
- [ ] Preserve the normal single-checkout flow as the default; do not require worktree registration for ordinary `init`, `daemon start`, or `mcp-config --json` usage.
