...
//    npm run artifact:private-rc -- --skip-full-validation
//    npm run artifact:private-rc -- --keep-temp
//    npm run artifact:private-rc -- --out ./my-artifacts
//    npm run artifact:private-rc -- --allow-diverged
//    npm run rc:private   (alias, same command)
...
//
//  This file was auto-generated by \`npm run artifact:private-rc\`.
//
...
function main() {
  ...
  try {
    ...
    fs.writeFileSync(checklistPath, [
      ...
      'npm init -y',
      `npm install /path/to/this-bundle/${tarballName}`,
      '```',
      `- [ ] Install from the tarball (see above)`,
      `- [ ] Run \`./node_modules/.bin/xurgo-atlas --help\` and verify usage text`,
      `- [ ] Run \`./node_modules/.bin/xurgo-atlas init --project-root . --project-id <project-id>\``,
      `- [ ] Run \`./node_modules/.bin/xurgo-atlas daemon start\``,
      `- [ ] Run \`./node_modules/.bin/xurgo-atlas status\``,
      `- [ ] Run \`./node_modules/.bin/xurgo-atlas mcp-config\` and note the endpoint`,
