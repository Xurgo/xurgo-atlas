...
statusVersion: 1
priority: high
currentFocus: "Packaging now ships a minimal allowlisted runtime package, guarded patch writes are narrowed to curated-owned docs, storage defaults prefer Xurgo Atlas roots with legacy docu-guard fallback discovery, and the latest runtime token cleanup now uses Atlas-branded internal markers while preserving intentional compatibility references"
nextActions:
